...
import {
    LayoutAnimation, YellowBox, Animated, View, PanResponder, Platform, UIManager, StatusBar, StyleSheet, PanResponderInstance
} from 'react-native';
...
class RowItem extends React.PureComponent<any, any> {
...
export class SortableFlatList extends Component<IProps, any> {
    static defaultProps: { scrollPercent: number; contentContainerStyle: {}; scrollSpeed: number };
    static _propsStack: any[];
        ...
        this._panResponder = PanResponder.create({
            onStartShouldSetPanResponderCapture: (evt, gestureState) => {
                ...
                if (Platform.OS === 'android' && !horizontal) {
                    const isTranslucent = (StatusBar as any)._propsStack.reduce(((acc, cur) => {
                        return cur.translucent === undefined ? acc : cur.translucent;
                    }), false);

                    const isHidden = (StatusBar as any)._propsStack.reduce(((acc, cur) => {
        ...
        return (
            ...
            >
                <KeyboardAwareFlatList
                    ...
                    innerRef={ref => this._flatList = ref}
